...
class App extends Component {
	...
	changeImageHandler = (action) => {
		...
		if (action === "back" && this.state.index > 0) {
			_index = this.state.index - 1;
		}
		this.setState({
			index: _index,
	...
	render() {
		...
		if (this.state.currImg.date) {
			InfoComponent = this.state.infoHidden ? (
				<ToggleButton
					type="arrowUp"
					clicked={this.infoBarHandler}
					label={"Show Bar"}
				/>
			) : (
				<ImageInfo
					style={{
						display: this.state.fullScreen ? "none" : "inherit",
					}}
					data={this.state.currImg}
					formatedDate={this.convertDateFormat(
						this.state.currImg.date
					)}
		...
		const currImgToDisplay = this.state.hd
			? this.state.currHDimg
			: this.state.currImg.url;

		return (
			<MuiThemeProvider theme={theme}>
				<div
					className={classes.App}
					style={{ backgroundImage: `url(${currImgToDisplay})` }}
				>
					<LeftDrawer
					...
					<Controls
						changeFullscreen={this.fullScreenHandler}
